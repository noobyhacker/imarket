// ISO-3166 alpha-2 country list. Flag emoji derived from the code
// (regional indicator symbols) so we don't hardcode 200 emojis.
...
/** "🇰🇷 South Korea" — falls back gracefully for null/unknown. */
export function getCountryLabel(code: string | null | undefined): string {
  if (!code) return 'Unspecified';
  const name = CODE_TO_NAME.get(code.toUpperCase());
  if (!name) return code;
